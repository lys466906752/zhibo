<?php
	...
	class Projects_model extends CI_Model
	{
		...
		private function tableName()
		{
			...
			return $this->db->dbprefix.'projects';
		...
		public function getRows()
		{
			...
			$query=$this->db->query("select * from `".$this->tableName()."` order by `updateTime` desc");
		...
		{
			$msg='';
			$res=$this->getOneRowForName($data['title']);
			if(!$res)
			{
				$_array=[
					'name'=>$data['title'],
					'timeOver'=>$data['startTime'],
					'endTime'=>0,
					'logo'=>'assets/images/0022005489636837_b.jpg',
					'avatar'=>'assets/images/0022005489636837_b.jpg',
					'nickname'=>$data['title'],
					'playUrl'=>$data['playUrl'],
					'pushUrl'=>$data['pushUrl'],
					'playFlvUrl'=>$data['playFlvUrl'],
					'honor'=>'官网',
					'backColor'=>'#40cddf',
					'bottomColor'=>'#a0e6ef',
					'mobileCodeShow'=>1,
				...
				if($this->db->insert($this->tableName(),$_array))
				{
					...
					$this->db->insert('pother',$_arr);
					
					//创建其他表，添加其他表数据
					$this->db->query("CREATE TABLE IF NOT EXISTS `mv_comments_".$insert_id."` (
					  ...
					  PRIMARY KEY (`id`),
					  KEY `admins` (`admins`)
					) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='对应的专题交流表' AUTO_INCREMENT=1");
					...
					$this->db->insert('ip',$_array);
					
					$_array=[
						'id'=>$insert_id,
					];
					$this->db->insert('results',$_array);
					
					
					$tableName='mv_visited_'.$insert_id;
					$this->db->query("CREATE TABLE IF NOT EXISTS `".$tableName."` (
					  ...
					  PRIMARY KEY (`id`)
					) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='访问明细表' AUTO_INCREMENT=1 ");
					
					$this->db->query("CREATE TABLE IF NOT EXISTS `mv_region_".$insert_id."` (
  ...
  PRIMARY KEY (`id`)
					...
					$this->db->query("INSERT INTO `mv_region_".$insert_id."` (`id`, `name`, `data`, `counts`) VALUES
		...
		{
			$msg='';
			$res=$this->getOneRowForNameNoId($data['title'],$data['id']);
			if(!$res)
			{
				$_array=[
					'name'=>$data['title'],
		...
		//修改频道主页的评论配置
		public function editOther($data,&$msg)
		{
			$msg='';
		...
		//修改交流的昵称
		public function editNickName($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
		...
		//修改观看页主题设置
		public function familyUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
			{
				$_array=[
					'mobileCodeShow'=>$data['mobileCodeShow'],
					'logo'=>$data['logoVal'],
					'bgFile'=>$data['bgFileVal'],
		...
		//修改引导图设置
		public function zbydtUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
			{
				$_array=[
					'mobileStartShow'=>$data['mobileStartShow'],
					'mobileStartFile'=>$data['mobileStartFile'],
					'updateTime'=>time()
				];
				if($this->db->update($this->tableName(),$_array,['id'=>$data['id']]))
				{
		...
		//修改倒计时信息
		public function djsUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
			{
				$data['timeOver']=='' || $data['timeOver']==0?$data['timeOver']=0:$data['timeOver']=strtotime($data['timeOver']);
				$_array=[
					'timeOverShow'=>$data['timeOverShow'],
					'timeOver'=>$data['timeOver'],
					'timeOverText'=>$data['timeOverText'],
					'updateTime'=>time()
				];
				if($this->db->update($this->tableName(),$_array,['id'=>$data['id']]))
				{
		...
		//修改显示人数
		public function gzrsxsszUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
			{
				$_array=[
					'playNumsSuccess'=>$data['playNumsSuccess'],
					'playNumsDefaults'=>$data['playNumsDefaults'],
					'playNumsStateType'=>$data['playNumsStateType'],
					'playNumsState'=>$data['playNumsState'],
					'updateTime'=>time()
				];
				if($this->db->update($this->tableName(),$_array,['id'=>$data['id']]))
				{
		...
		//更新背景图
		public function zbckbjUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
		...
		//修改公众号
		public function gzhszUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
			{
				$_array=[
					'gongzhonghaoState'=>$data['gongzhonghaoState'],
					'gongzhonghaoShow'=>$data['gongzhonghaoShow'],
					'gongzhonghaoFile'=>$data['gongzhonghaoFile'],
					'gongzhonghaoName'=>$data['gongzhonghaoName'],
					'updateTime'=>time()
				];
				if($this->db->update($this->tableName(),$_array,['id'=>$data['id']]))
				{
		...
		//更新视频logo水印
		public function logoUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
			{
				$_array=[
					'mvLogoState'=>$data['mvLogoState'],
					'mvLogoStateWay'=>$data['mvLogoStateWay'],
					'mvFile'=>$data['mvFile'],
					'updateTime'=>time()
				];
				if($this->db->update($this->tableName(),$_array,['id'=>$data['id']]))
		...
		//修改分享设置
		public function shareUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
			{
				$_array=[
					...
					'shareContents'=>$data['shareContents'],
		...
		//修改广告的处理方法
		public function zdygglUpdate($data,&$msg)
		{
			$msg='';
			$res=$this->getOneRow($data['id']);
			if($res)
			{
				...
				if($topVal!='')
				{
					$arrs=explode('____',$topVal);
					foreach($arrs as $v)
					{
						if(trim($v)!='')
						{
							$arrItem=explode('||||',trim($v));
							//拼接数据	
							$_Json['topAd'][$a]['hashId']=$arrItem[0];
							$_Json['topAd'][$a]['file']=$arrItem[1];
							$_Json['topAd'][$a]['url']=$arrItem[2];
							$_Json['topAd'][$a]['pv']=$this->getPv($res['ads'],$arrItem[0],'topAd');
							$a++;
						}	
					}	
				}
				//循环处理第二个数组
				$middleVal=$data['middleVal'];
				if($middleVal!='')
				{
					$arrs=explode('____',$middleVal);
					foreach($arrs as $v)
					{
						if(trim($v)!='')
						{
							$arrItem=explode('||||',trim($v));
							//拼接数据	
							$_Json['middleAd'][$b]['hashId']=$arrItem[0];
							$_Json['middleAd'][$b]['file']=$arrItem[1];
							$_Json['middleAd'][$b]['url']=$arrItem[2];
							$_Json['middleAd'][$b]['pv']=$this->getPv($res['ads'],$arrItem[0],'middleAd');
							$b++;
						}	
					}	
				}
		...
		protected function getPv($ads,$hashId,$k)
		{
			if($ads=='')
			{
				return 0;	
			}
			$array=json_decode($ads,true);
			...
			if(isset($array[$k]))
			{
				...
				{
					if($array[$k][$i]['hashId']==$hashId)
					{
						return 	isset($array[$k][$i]['pv']) && $array[$k][$i]['pv']!=''?intval($array[$k][$i]['pv']):0;
					}	
				}	
				return 0;
			}
		...
		public function del($id,&$msg)
		{
			...
			{
				$this->db->query("drop table if exists `mv_region_".$id."`");
				$this->db->query("drop table if exists `mv_comments_".$id."`");
				$this->db->query("drop table if exists `mv_visited_".$id."`");
				$this->db->query("delete from `mv_cnums` where `id`='$id'");
				$this->db->query("delete from `mv_ip` where `id`='$id'");
				$this->db->query("delete from `mv_pother` where `id`='$id'");
				$this->db->query("delete from `mv_results` where `id`='$id'");
				return true;
			}
