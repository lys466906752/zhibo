	...
	function add_admin()
	{
		if(form_loads==1)
		{
			...
			else
			{
				layer.closeAll();
				form_loads=2;	
				$.ajax({url:"<?php echo admin_url();?>home/update/<?php echo $res['id'];?>", 
				type: 'POST', 
...
?>
  <tr>
    <td height="60" colspan="2" align="left" valign="middle" style="padding-top:15px;"><h2>修改直播频道</h2></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="83%" height="55" align="left" valign="middle"><input type="text" name="title" id="title" style="width:90%; height:30px; line-height:30px; padding-left:5px; padding-right:5px; border:#e4e4e4 1px solid; border-radius:3px;" placeholder="请输入频道名称，长度最多为50字"  value="<?php echo $res['name'];?>" /></td>
        </tr>
        <tr>
          <td height="55" align="center" valign="middle">直播开始时间：</td>
          <td height="55" align="left" valign="middle"><input type="text" name="startTime" id="startTime" style="width:90%; height:30px; line-height:30px; padding-left:5px; padding-right:5px; border:#e4e4e4 1px solid; border-radius:3px;" placeholder="请选择直播开始时间，选填项"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" class="Wdate" readonly="readonly"  value="<?php echo $res['timeOver']=='' || $res['timeOver']==0?'':date('Y-m-d H:i:s',$res['timeOver']);?>" /></td>
        </tr>
        <tr>
          <td height="55" align="center" valign="middle">直播视频地址：</td>
          <td height="55" align="left" valign="middle"><input type="text" name="playUrl" id="playUrl" style="width:90%; height:30px; line-height:30px; padding-left:5px; padding-right:5px; border:#e4e4e4 1px solid; border-radius:3px;" placeholder="请填写视频直播地址，从腾讯云后台即可查找，如有问题，联系技术索取，选填项"  value="<?php echo $res['playUrl'];?>" /></td>
        </tr>
        <tr>
          <td height="55" align="center" valign="middle">直播视频地址(flv)：</td>
          <td height="55" align="left" valign="middle"><input type="text" name="playFlvUrl" id="playFlvUrl" style="width:90%; height:30px; line-height:30px; padding-left:5px; padding-right:5px; border:#e4e4e4 1px solid; border-radius:3px;" placeholder="请填写视频直播地址，从腾讯云后台即可查找，如有问题，联系技术索取，选填项"  value="<?php echo $res['playFlvUrl'];?>" /></td>
        </tr>
        <tr>
          <td height="55" align="center" valign="middle">视频推流地址：</td>
          <td height="55" align="left" valign="middle"><input type="text" name="pushUrl" id="pushUrl" style="width:90%; height:30px; line-height:30px; padding-left:5px; padding-right:5px; border:#e4e4e4 1px solid; border-radius:3px;" placeholder="请填写视频推流地址，从腾讯云后台即可查找，如有问题，联系技术索取，选填项" value="<?php echo $res['pushUrl'];?>" /></td>
        </tr>
        <tr>
          <td height="50" colspan="2" align="center" valign="middle"><input type="submit" name="button" id="button" value=" 确 认 并 更 新" onclick="add_admin();" /></td>
        </tr>
      </table>
