<?php
	...
	if(is_array($top) && !empty($top))
	{
		$array=$top;
?>
<input type="hidden" class="topValue" value="<?php echo $top['id'];?>" />
<div id="topInner">
<div style="width:100%; float:left; height:auto; border-bottom:#CCC 1px solid;" class="listComment_<?php echo $top['id'];?>">
  ...
  <tr>
    ...
    <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td width="92%" style="padding-top:15px;font-size:12px;"><span style="color:#09C;"><?php echo $array['nickname'];?></span>
          <?php
        	if($array['honor']!='')
			{
		?>
		...
		?>
          <span style="float:right;color:#999; padding-right:10px;"> <a href="javascript:zanyizan('<?php echo $array['id'];?>');" style="display:inline-block; width:auto; margin-right:20px;"> <i class="iconfont" style="color:#999;font-size:10px;<?php echo substr_count($array['upJson'],'Admin:'.$this->adminInfo['id'].',')>0?'font-weight:blod;color:#cc0000;':'';?>">
            <label id="zyz_<?php echo $array['id'];?>"><?php echo $array['up'];?></label>
            &#xeaa2;</i></a> <?php echo date('Y-m-d H:i:s',$array['time']);?> </span></td>
      </tr>
      <tr>
        <td style="line-height:25px;font-size:13px;color:#666; padding-top:10px; padding-bottom:3px;font-size:12px;padding-right:10px;" class="sodo"><?php echo get_ubb($array['contents']);?> <span style="float:right;display:none;"> <a href="javascript:huifu('<?php echo $array['id'];?>','<?php echo '回复 '.str_replace("'","\'",$array['nickname']);?>：');" style="border:#09C 1px solid; display:inline-block; width:auto; padding-left:5px; padding-right:5px; line-height:18px; text-align:center;color:#09C; font-size:12px; border-radius:3px;">回复</a>
          <?php
            	if($array['top']==2)
				{
			?>
          <a href="javascript:noDing('<?php echo $array['id'];?>');" style="border:#09C 1px solid; display:inline-block; width:auto; padding-left:5px; padding-right:5px; line-height:18px; text-align:center;color:#09C; font-size:12px; border-radius:3px;">取消置顶</a>
          <?php
				}
			?>
          <?php
      ...
      <?php
		...
		?>
      <tr>
        <td align="right" valign="middle" style="padding-bottom:10px;min-height:30px;padding-right:10px;"><?php
...
<?php
	...
	{
?>
<input type="hidden" class="topValue" value="" />
<div style="width:100%; float:left; height:auto; border-bottom:#CCC 1px solid;" id="topInner">
</div>
