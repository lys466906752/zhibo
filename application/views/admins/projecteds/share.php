            	...
            	<table width="100%" height="22" border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td width="58%" align="center" valign="top"><table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="18%" height="53" align="center" valign="middle">分享：</td>
                        <td width="82%" height="53" align="left" valign="middle"><select name="shareState" id="shareState"  style="width:250px;height:28px; line-height:28px; border:#CCC 1px solid; border-radius:5px; padding-left:3px; padding-right:3px; outline:none;" >
                          <option value="2" <?php if($res['shareState']==2){echo 'selected';}?>>开启</option>
                          <option value="1" <?php if($res['shareState']==1){echo 'selected';}?>>关闭</option>
                        </select></td>
                      </tr>
                      <tr>
                        <td height="53" align="center" valign="top" style="padding-top:15px;">标题：</td>
                        <td height="53" align="left" valign="middle"><input type="text" name="shareTitle" id="shareTitle" value="<?php echo $res['shareTitle'];?>"  style="width:450px;height:28px; line-height:28px; border:#CCC 1px solid; border-radius:5px; padding-left:3px; padding-right:3px; outline:none;" /></td>
                      </tr>
                      
                      <tr>
                        <td height="53" align="center" valign="middle">内容：</td>
                        <td height="53" align="left" valign="middle"><input type="text" name="shareContents" id="shareContents" value="<?php echo $res['shareContents'];?>"  style="width:450px;height:28px; line-height:28px; border:#CCC 1px solid; border-radius:5px; padding-left:3px; padding-right:3px; outline:none;"  /></td>
                      </tr>
                    </table></td>
                  </tr>
                  <tr>
                    <td align="center" valign="middle" style="padding-top:20px;"><a href="javascript:subEdit();" style="width:auto; padding-left:25px; padding-right:25px; line-height:28px; background:#09C;color:#fff; text-align:center; display:inline-block; border-radius:4px;"> 提 交 </a></td>
                  </tr>
                </table>
            </div>
        </div>
...
function subEdit()
{
	if(form_loads==1)
	{
		...
		var shareTitle=$("#shareTitle").val().replace(/(^\s*)|(\s*$)/g,"");
		var shareState=$("#shareState").val().replace(/(^\s*)|(\s*$)/g,"");
		var shareContents=$("#shareContents").val().replace(/(^\s*)|(\s*$)/g,"");

	
		layer.closeAll();
		form_loads=2;	
		$.ajax({url:"<?php echo admin_url();?>projecteds/shareUpdate/<?php echo $res['id'];?>", 
		type: 'POST', 
		data:{shareTitle:shareTitle,shareState:shareState,shareContents:shareContents}, 
		dataType: 'html', 
		timeout: 15000, 
			error: function(){
				layer.closeAll();
