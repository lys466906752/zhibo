            	...
            	<table width="100%" height="22" border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td width="58%" align="center" valign="top"><table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="28%" height="53" align="center" valign="middle">背景图预览：</td>
                        <td width="72%" height="53" align="left" valign="middle" id="playBgFileInner">
                        	<?php
                            	if($res['playBgFile']!='')
								{
							?>
                            	<img src="/<?php echo $res['playBgFile'];?>" style="width:480px;"  />
                            <?php
								}
								else
								{
...
function subEdit()
{
	if(form_loads==1)
	{
		...
		var playBgFile=$("#mobileStartFile").val().replace(/(^\s*)|(\s*$)/g,"");
	
		layer.closeAll();
		form_loads=2;	
		$.ajax({url:"<?php echo admin_url();?>projecteds/zbckbjUpdate/<?php echo $res['id'];?>", 
		type: 'POST', 
		data:{playBgFile:playBgFile}, 
		dataType: 'html', 
		timeout: 15000, 
			error: function(){
				layer.closeAll();
