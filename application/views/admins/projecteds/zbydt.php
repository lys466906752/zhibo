            	...
            	<table width="100%" height="22" border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td width="58%" align="center" valign="top"><table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="28%" height="53" align="center" valign="middle">直播引导图：</td>
                        <td width="72%" height="53" align="left" valign="middle"><select name="mobileStartShow" id="mobileStartShow" onchange="subEdit();"  style="width:250px;height:28px; line-height:28px; border:#CCC 1px solid; border-radius:5px; padding-left:3px; padding-right:3px; outline:none;" >
                          <option value="2" <?php if($res['mobileStartShow']==2){echo 'selected';}?>>显示</option>
                          <option value="1" <?php if($res['mobileStartShow']==1){echo 'selected';}?>>关闭</option>
                        </select></td>
                      </tr>
                      <tr>
                        <td height="53" align="center" valign="top" style="padding-top:15px;">直播引导图片：</td>
                        ...
                        <input type="hidden" id="mobileStartFile" name="mobileStartFile" value="<?php echo $res['mobileStartFile'];?>" />
                        <p style="color:#999;font-size:12px; line-height:24px;">此图片显示在进入直播页面之前，用于展示直播相关介绍内容<br />图片大小2M以内<br />图片分辨率1080*1740像素<br />图片格式：png、jpg、jpeg；</p>
                      ...
                      <tr>
                        <td height="53" align="center" valign="middle">
                        	<a href="javascript:subEdit();" style="width:auto; padding-left:25px; padding-right:25px; line-height:28px; background:#09C;color:#fff; text-align:center; display:inline-block; border-radius:4px;"></a>
                        </td>
                        <td height="53" align="left" valign="middle">
                        <p>
                        	<img src="<?php echo admin_url();?>projecteds/ydtShowCode/<?php echo $res['id'];?>" width="150" height="150" style="border:#CCC 1px dotted;" />
                        </p>
                        <p>
                        	扫一扫 查看手机效果
                        </p>
                    ...
                    <td width="42%" align="center" valign="top">
                   	  ...
                   	  <div style="width:90%;text-align:center; padding-top:10px; float:left; min-height:400px;" id="testShow">
                        	
                           
                            
                            <div style="width:272px; height:541px; margin:0 auto; background:url(/assets/images/act_guide.png) no-repeat; text-align:center;" id='ydtInner'>
                            <?php
                            	if($res['mobileStartFile']!='')
								{
							?>
                            <img src="/<?php echo $res['mobileStartFile'];?>" width="234" height="375" style="margin-top:110px; margin-left:3px;" />
                            <?php
								}
							?>	
                            </div>
...
function subEdit()
{
	if(form_loads==1)
	{
		var mobileStartShow=$("#mobileStartShow").val().replace(/(^\s*)|(\s*$)/g,"");
		var mobileStartFile=$("#mobileStartFile").val().replace(/(^\s*)|(\s*$)/g,"");
	
		layer.closeAll();
		form_loads=2;	
		$.ajax({url:"<?php echo admin_url();?>projecteds/zbydtUpdate/<?php echo $res['id'];?>", 
		type: 'POST', 
		data:{mobileStartShow:mobileStartShow,mobileStartFile:mobileStartFile}, 
		dataType: 'html', 
		timeout: 15000, 
			error: function(){
				layer.closeAll();
...
function stopUpload(res)
{
	...
	if(res.indexOf("|")>=0)
	{
		...
		if(arr[0]==100)
		{
			...
			$('#ydtInner').html('<img src="/' + arr[1] + '" width="234" height="375" style="margin-top:110px; margin-left:3px;" />');
			subEdit();
		}
		else if(arr[0]==200)
		{
