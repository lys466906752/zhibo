...
<li id="comments_<?php echo $array['id'];?>">
    ...
    <?php
			...
			if(!in_array(date('Y_m_d',$array['time']),$dataArrayer))
			{
				$dataArrayer[]=date('Y_m_d',$array['time']);
			}
	?>
...
<?php
	...
	if($nextId!='' && $nextId!=0 && is_numeric($nextId) && $nextId!=$minIder+1)
	{
		for($z=$nextId-1;$z>$minIder;$z--)
		{
?>
<label id="cm_<?php echo $z;?>" style="display:none;"></label>
<?php
		}
	}
